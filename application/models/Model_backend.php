...
class Model_backend extends CI_Model {
	...
	function report_filter()
	{
		if ($this->input->post('jenis')==='all') {
			...
			UNION
			SELECT nama_instansi,tanggal_kunjungan,nama_ketua,wilayah,sub_lokasi,no_kunjungan,'',pria,wanita,'','' FROM rombongan WHERE `tanggal_kunjungan` >= '" . $this->input->post('tgl_awal') . "' AND `tanggal_kunjungan` <= '" . $this->input->post('tgl_akhir') . "'
			");
			return $hasil->result();
